...
#
# What it does:
#   1. Bumps the trailing version segment in HealerInRange.toc
#      (1.2.N → 1.2.(N+1)). Done first so a malformed .toc fails
#      fast before anything destructive happens to the live install.
#   2. Wipes /Applications/World of Warcraft/_retail_/Interface/AddOns/HealerInRange
...
#   libs/             — embedded Ace3 / LibStub / LibRangeCheck / LibSharedMedia (required at runtime)
#   logo.tga          — addon logo
...
DEST="/Applications/World of Warcraft/_retail_/Interface/AddOns/HealerInRange"
