...
#
# Output: ../HealerInRange-v<version>.zip   (sibling of this repo, in the
#         shared addons working dir).
#
# The zip contains a single top-level folder `HealerInRange/` so users on
...
#
# Kept:
#   .toc / .xml / .lua, libs/ (Ace3/LibStub/LibRangeCheck/LibSharedMedia),
#   logo.tga, LICENSE
...
ZIP_NAME="HealerInRange-v${version}.zip"
ZIP_PATH="$OUT_DIR/$ZIP_NAME"

echo "Packaging HealerInRange v${version}"
echo "   from: $SRC"
echo "   to:   $ZIP_PATH"

# --- Stage in a temp dir so the zip has a clean HealerInRange/ root -------
STAGE="$(mktemp -d)"
...
rm -f "$ZIP_PATH"
( cd "$STAGE" && COPYFILE_DISABLE=1 zip -rXq "$ZIP_PATH" "HealerInRange" )
